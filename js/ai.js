...
function aiShortPushThr(p, stackBB){
  ...
  else if(sid==='maniac' && /^(CO|BTN|SB)$/.test(p.pos)) thr=Math.min(0.88, thr*2.0);
  else if(sid==='shark' && /^(CO|BTN)$/.test(p.pos)) thr=Math.min(0.50, thr*1.12);
  return Math.min(0.90, thr+press*(p.style?.adapt||0)*0.12);
}
function aiCanValueRaise(p){
  if(!p.style) return true;
  return (handPct[holeCode(p.hole)]||1)<=(p.style.raiseCap||0.22);
}
