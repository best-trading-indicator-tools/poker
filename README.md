...
- **Full-combo postflop solver**: b-inary's open-source Rust CFR engine resolves covered heads-up postflop spots in a Web Worker from an independent preflop baseline, carries equilibrium reach through later actions/runouts, shows the mixed strategy and per-action EVs, and overrides the coach only after strict line replay and convergence checks
- **Stats & training**: post-hand feedback, session + lifetime stats (persisted), full hand-history export to JSON, plus a readable `.txt` audit export of the latest 10 hands with action context and AI-coach/solver metadata for recommendation debugging
...
- **Strict preflop coverage**: the bundled data is still a chart abstraction, not a complete frequency blueprint. Exact-provider coverage is therefore limited to 80–120 BB, ante-free cash single-raised pots whose RFI and one call/BB-defence nodes exist in `charts.js`. Limps, unsupported blind defences, 3-bets/squeezes, all-ins, tournaments, off-policy hero hands, and resumed hands do not receive a solver label. This removes personality leakage without pretending the approximate chart file is universal preflop GTO.
...
- **Honest fallbacks**: preflop remains chart-backed; uncovered preflop lines, a missing prior-street equilibrium reach, multiway pots, payout-sensitive tournament play, all-in nodes, nonconverged or non-replayable lines, oversized browser trees, and unavailable WASM use the range-aware/ICM-aware heuristic provider and are labeled as fallbacks rather than GTO.
