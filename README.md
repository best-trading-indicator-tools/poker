...
- **Full-combo postflop solver**: b-inary's open-source Rust CFR engine resolves heads-up postflop spots in a Web Worker from the app's inferred ranges, shows the mixed strategy and per-action EVs, and overrides the coach only after strict line replay and convergence checks
...
- **Honest fallbacks**: preflop remains chart-backed; multiway pots, payout-sensitive tournament play before heads-up, all-in nodes, nonconverged or non-replayable lines, oversized browser trees, and unavailable WASM use the range-aware/ICM-aware heuristic provider and are labeled as fallbacks rather than GTO.
